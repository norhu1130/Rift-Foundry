# Phase 0 Tasks — Rift Foundry (LoL Build Recommendation Engine)

목표: 한 챔피언과 한 개의 결정론적 전투 시나리오에서 패치가 잠긴 로컬
...
- 현재 증거:
  ...
  - 173개 챔피언 물리 모듈을 성숙도별 하위 폴더로 재배치하고 `manifest.py`의
    `module_name`을 일괄 갱신했다(`/tmp/reorg.py`, exact-string 치환 후
...
`P0-051 추천 파이프라인 연결`의 증거 차단을 해소하고, 생성되는 세 분기에
`P0-052 provenance`를 연결한다. P0-041은 증거 검색과 실측
프로토콜을 완료했지만 16.17.1 클라이언트 관측 대기 상태다. P0-035는 자동
검증을 통과했지만 잠긴 클라이언트의 사람 실측값이 없어 승격만 `BLOCKED`다.
P0-023은 과거 두 아이템의
개별 시간 측정치가 없어 중앙값 완료 조건만 `BLOCKED`이며, 이 측정 결손은
독립적인 계산 엔진 구현을 막지 않는다.

최근 작업은 P1-065(5대5 한타 확장과 교전 전제 정정)에 집중됐고 위 P0 차단
항목은 그대로다. P0-041과 P0-035는 16.17.1 클라이언트 실측을 기다리는
상태여서 코드로 풀 수 없다. M0 게이트를 열려면 실측 세션이 선행돼야 한다.
