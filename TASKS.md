...
1. `ruff format` 일괄 적용 — 동작 변경 없는 단독 커밋으로 ADR-0001 게이트를
   실제로 켠다.
2. P1-064 남은 31개 챔피언 승격과 광역기 Cog의 `MULTI_TARGET` 확장.
3. D-004 숙련자 빌드 코퍼스 파일럿 — 엔진 결과를 외부 정성 근거와 대조할
   첫 수단이다.
4. P1-067의 남은 과제: 방어 분기 우선순위 설계 검토(P1-068에서 DEFENSE는
