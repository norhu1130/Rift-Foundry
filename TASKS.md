...
### P1-073 — 개선 점검의 남은 과제 일괄 처리

- 상태: `DONE_NON_RELEASE`
- 우선순위: `P1`
- 선행: P1-072
- 배경: `reports/improvement-review.md`에 후속 과제로 남긴 항목을 모두 진행하라는
  요청(룬은 사용자 요청으로 제외 — `RUNES_NOT_MODELED` blocker 유지).
- 처리 내용(자세한 근거는 보고서 18~24차):
  - 공격 속도 상한 3.003(위키) 적용, 강인함 그룹 사이 합연산.
  - 킷 슬로우 추격 반영: 기본 hook이 행동 계획의 슬로우를 사거리·지속 시간
    규칙으로 반영(40개 Cog, Trundle·Karthus·Gragas 추격 실패 0).
  - Jhin 4타 출력 인자 순서 버그(추천 크래시) 수정, 피해 출력 타입 검증과 전 Cog
    구조 테스트 추가.
  - 점화: 선택 항목, 확인된 고통스러운 상처만 반영, 피해 공식은 blocker.
  - 치료 감소 하위 부품: 평가 전용, 첫 귀환 비교(Darius vs Aatrox → 처형인의
    대검).
  - 짝 없는 후속 이벤트 22개 분류(Zaahen W 1개 표시).
  - 레벨별 스킬 랭크: 표준 레벨업 규칙 + 원본 DataValue 조회로 57개 Cog의
    랭크 리터럴 202개 전환(레벨 13 결과 불변, worktree 사전 검증).
- 남은 한계: 쿨타임·시전 시점은 레벨 13 고정(`*_MODEL_LEVEL_UNSUPPORTED` 유지),
  Ashe·Elise·Udyr와 원본 배열에 유일하게 대응하지 않는 리터럴은 미전환, 점화
  피해 수치 미확인, 후보별 지표 딕셔너리 메모리 절감은 측정 후 결정.
