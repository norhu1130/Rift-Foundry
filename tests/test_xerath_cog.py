"""Focused regressions for the locked Xerath champion Cog."""
...
def _context(
    *, as_actor: bool = True, item_stats: dict[str, Decimal] | None = None
    ...
    """Build a level-13 Xerath-versus-Garen encounter.

    :param as_actor: Place Xerath in the actor role when true.
    :param item_stats: Optional permanent modifiers applied to Xerath.
    :return: Deterministic eight-second participant context.
    """
    registry = create_default_registry(ROOT)
    xerath = registry.require_cog("Xerath")
    garen = registry.require_cog("Garen")
    return ParticipantContext(
        EntityId.ACTOR if as_actor else EntityId.TARGET,
        EntityId.TARGET if as_actor else EntityId.ACTOR,
        xerath.snapshot(level=13, item_stats=item_stats),
...
def test_xerath_metadata_stun_and_ramping_shots_are_explicit() -> None:
    """Require evidence, the E stun, and R's four escalating shots."""
    cog = create_default_registry(ROOT).require_cog("Xerath")
    ...
    shots = sorted(
        (e for e in plan.events if "R_RITE_OF_THE_ARCANE" in e.id),
    ...
    assert damages == sorted(damages)  # strictly non-decreasing ramp
...
def test_xerath_reaction_plan_exposes_the_e_stun() -> None:
    """Expose the Shocking Orb stun as a control window."""
    cog = create_default_registry(ROOT).require_cog("Xerath")
...
def test_xerath_is_role_symmetric_between_actor_and_target() -> None:
    """Produce the same event shape whether Xerath is the actor or the target."""
    cog = create_default_registry(ROOT).require_cog("Xerath")
