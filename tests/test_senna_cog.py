"""Focused regressions for the locked Senna champion Cog."""
...
def _context(*, as_actor: bool = True) -> ParticipantContext:
    """Build a level-13 Senna-versus-Garen encounter.

    :param as_actor: Place Senna in the actor role when true.
    :return: Deterministic eight-second participant context.
    """
    registry = create_default_registry(ROOT)
    cog = registry.require_cog("Senna")
...
def test_senna_is_modeled_with_locked_evidence() -> None:
    """Require evidence, the duel capabilities, and a deterministic plan."""
    cog = create_default_registry(ROOT).require_cog("Senna")
...
def test_senna_carries_no_self_heal_or_shield_in_a_duel() -> None:
    """Keep ally-only heals and shields out of a two-participant encounter."""
    plan = create_default_registry(ROOT).require_cog("Senna").build_action_plan(_context())

    assert not _outputs(plan, HealOutput)
    assert not _outputs(plan, ShieldOutput)
    assert "SENNA_Q_HEAL_AND_R_SHIELD_REQUIRE_ALLIES" in plan.blockers


def test_senna_is_role_symmetric_between_actor_and_target() -> None:
    """Produce the same event shape whether Senna is the actor or the target."""
    cog = create_default_registry(ROOT).require_cog("Senna")
