"""Focused regressions for the locked Seraphine champion Cog."""
...
def _context(*, as_actor: bool = True) -> ParticipantContext:
    """Build a level-13 Seraphine-versus-Garen encounter.

    :param as_actor: Place Seraphine in the actor role when true.
    :return: Deterministic eight-second participant context.
    """
    registry = create_default_registry(ROOT)
    cog = registry.require_cog("Seraphine")
...
def test_seraphine_is_modeled_with_locked_evidence() -> None:
    """Require evidence, the duel capabilities, and a deterministic plan."""
    cog = create_default_registry(ROOT).require_cog("Seraphine")
...
def test_seraphine_shields_herself_but_does_not_heal_without_allies() -> None:
    """Grant the rank-one Surround Sound shield and exclude the ally heal."""
    plan = create_default_registry(ROOT).require_cog("Seraphine").build_action_plan(_context())

    assert [s.amount for s in _outputs(plan, ShieldOutput)] == [Decimal(60)]
    assert not _outputs(plan, HealOutput)


def test_seraphine_is_role_symmetric_between_actor_and_target() -> None:
    """Produce the same event shape whether Seraphine is the actor or the target."""
    cog = create_default_registry(ROOT).require_cog("Seraphine")
